...
extern int zdo_device_announce_handler(const wpan_envelope_t FAR* envelope, void FAR* context);
...
wpan_cluster_table_entry_t zdo_cluster_table[] =
{
   { ZDO_DEVICE_ANNCE, &zdo_device_announce_handler, NULL, WPAN_CLUST_FLAG_INOUT },
...
int walker_init(wpan_dev_t* dev, addr64* target,  uint16_t network_addr, uint16_t flags)
{
   ...
   walker.target.network = network_addr;  //WPAN_NET_ADDR_UNDEFINED;
